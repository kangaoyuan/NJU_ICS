...
static inline def_EHelper(lidt) {
  ...
  if(s->isa.is_operand_size_16) 
    cpu.idtr_base = vaddr_read(*s->isa.mbase+s->isa.moff+2,4)&0x00ffffff;
  else cpu.idtr_base= vaddr_read(*s->isa.mbase+s->isa.moff+2,4);
  print_asm_template1(lidt);
}

static inline def_EHelper(lgdt) {
  cpu.gdtr_limit = (*ddest)>>16;
  if(s->isa.is_operand_size_16) 
    cpu.gdtr_base = vaddr_read(*s->isa.mbase+s->isa.moff+2,4)&0x00ffffff;
  else cpu.gdtr_base= vaddr_read(*s->isa.mbase+s->isa.moff+2,4);
